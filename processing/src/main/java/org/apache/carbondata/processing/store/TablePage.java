  ...
  TablePage(CarbonFactDataHandlerModel model, int pageSize) throws MemoryException {
    ...
    for (int i = 0; i < dictDimensionPages.length + noDictDimensionPages.length; i++) {
      ...
      if (ColumnType.GLOBAL_DICTIONARY == columnType
        ...
        page = ColumnPage.newPage(dimensionSpec, DataTypes.INT, pageSize);
        ...
        if (null != localDictionaryGenerator) {
          page = ColumnPage
              .newLocalDictPage(spec, dataType, pageSize, localDictionaryGenerator);
        } else {
      ...
      for (int i = 0; i < noDictAndComplex.length; i++) {
        if (i < noDictionaryCount) {
          noDictDimensionPages[i].putData(rowId,
              Arrays.equals(noDictAndComplex[i], CarbonCommonConstants.MEMBER_DEFAULT_VAL_ARRAY) ?
                  null :
                  noDictAndComplex[i]);
        } else {
          // complex columns
