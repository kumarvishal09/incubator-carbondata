...
public abstract class IndexStorageEncoder extends ColumnPageEncoder {
   ...
   * index generator
   */
  protected PageIndexGenerator pageIndexGenerator;
  /**
   * compressed data
   */
  protected byte[] compressedDataPage;
  ...
  /**
   * data type select for binary column
   */
  protected DataType selectedDataType;

  /**
   * for binary type if all data is of same length
   */
  protected boolean isEqualLength;

  /**
   * for binary type data length when all data size is same
   */
  protected int eachDataLength;
  ...
  IndexStorageEncoder(boolean isBinaryType, KeyGenerator keyGenerator, List<Encoding> encoding) {
    this.isBinaryType = isBinaryType;
  ...
  protected List<ByteBuffer> buildEncoderMeta(ColumnPage inputPage) throws IOException {
    if (this.isBinaryType) {
      ...
      ByteArrayOutputStream stream = new ByteArrayOutputStream();
      DataOutputStream out = new DataOutputStream(stream);
      out.writeBoolean(isEqualLength);
      if (!isEqualLength) {
        out.writeByte(selectedDataType.getId());
      } else {
        out.writeInt(eachDataLength);
      }
